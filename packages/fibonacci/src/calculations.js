...
export const calculateFibonacciRetracement = (swingHigh, swingLow, direction = 'up') => {
    const range = swingHigh - swingLow;
    return FIBONACCI_RETRACEMENT_LEVELS.map((level) => {
        const price = direction === 'up' ? swingHigh - range * level : swingLow + range * level;
        return {
            level,
            price,
...
export const calculateFibonacciExtension = (point1, point2, point3) => {
...
export const calculateProjectionLevels = (swingLow, swingHigh, direction) => {
    const range = swingHigh.price - swingLow.price;
    if (range <= 0)
        return [];
    console.log('[FIBO DEBUG] FIBONACCI_ALL_LEVELS:', FIBONACCI_ALL_LEVELS);
