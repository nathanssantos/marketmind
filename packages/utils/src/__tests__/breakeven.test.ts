...
describe('calculateBreakevenPrice', () => {
  ...
  it('returns a price above entry for LONG', () => {
    ...
    expect(be).toBeCloseTo(50_050.025, 2);
  });
  ...
  it('returns a price below entry for SHORT', () => {
    ...
    expect(be).toBeCloseTo(49_950.025, 2);
  });
  ...
  it('uses Binance Futures VIP 0 (0.05%) as the default takerRate', () => {
    const beExplicit = calculateBreakevenPrice({ entryPrice: ENTRY, side: 'LONG', takerRate: 0.0005 });
