...
describe('useROCRenderer', () => {
  ...
  const mockRocData = { values: [2.5, -1.2, 3.8, -0.5, 1.5] };
  ...
  describe('render', () => {
    it('should not render when manager is null', () => {
      const { result } = renderHook(() => useROCRenderer({ manager: null, rocData: mockRocData, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should not render when disabled', () => {
      const { result } = renderHook(() => useROCRenderer({ manager: mockManager, rocData: mockRocData, colors: mockColors, enabled: false }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should not render when rocData is null', () => {
      const { result } = renderHook(() => useROCRenderer({ manager: mockManager, rocData: null, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should render ROC when enabled with valid data', () => {
      const { result } = renderHook(() => useROCRenderer({ manager: mockManager, rocData: mockRocData, colors: mockColors, enabled: true }));
      result.current.render();
  ...
  describe('default values', () => {
    it('should use default enabled of true when not specified', () => {
      const { result } = renderHook(() => useROCRenderer({ manager: mockManager, rocData: mockRocData, colors: mockColors }));
      result.current.render();
      expect(mockCtx.save).toHaveBeenCalled();
    });
  });
