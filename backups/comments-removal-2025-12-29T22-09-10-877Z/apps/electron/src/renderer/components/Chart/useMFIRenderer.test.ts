...
describe('useMFIRenderer', () => {
  ...
  const mockColors = {
    ...
    text: '#D9D9D9',
    axisLabel: '#848E9C',
    mfi: { line: '#9c27b0', zone: 'rgba(156, 39, 176, 0.1)' },
  };

  const mockMfiData = [50, 65, 75, 40, 55];
  ...
  describe('render', () => {
    it('should not render when manager is null', () => {
      const { result } = renderHook(() => useMFIRenderer({ manager: null, mfiData: mockMfiData, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should not render when disabled', () => {
      const { result } = renderHook(() => useMFIRenderer({ manager: mockManager, mfiData: mockMfiData, colors: mockColors, enabled: false }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should not render when mfiData is null', () => {
      const { result } = renderHook(() => useMFIRenderer({ manager: mockManager, mfiData: null, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should render MFI when enabled with valid data', () => {
      const { result } = renderHook(() => useMFIRenderer({ manager: mockManager, mfiData: mockMfiData, colors: mockColors, enabled: true }));
      result.current.render();
  ...
  describe('default values', () => {
    it('should use default enabled of true when not specified', () => {
      const { result } = renderHook(() => useMFIRenderer({ manager: mockManager, mfiData: mockMfiData, colors: mockColors }));
      result.current.render();
      expect(mockCtx.save).toHaveBeenCalled();
    });
  });
