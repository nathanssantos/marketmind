...
import { useParabolicSARWorker } from './useParabolicSARWorker';

describe('useParabolicSARWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => useParabolicSARWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => useParabolicSARWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => useParabolicSARWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => useParabolicSARWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => useParabolicSARWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => useParabolicSARWorker(mockKlines, enabled), {
      initialProps: { enabled: false },
    });
    expect(() => rerender({ enabled: true })).not.toThrow();
  });

  it('should accept custom step and max', () => {
    expect(() => renderHook(() => useParabolicSARWorker(mockKlines, true, 0.02, 0.2))).not.toThrow();
  });
});
