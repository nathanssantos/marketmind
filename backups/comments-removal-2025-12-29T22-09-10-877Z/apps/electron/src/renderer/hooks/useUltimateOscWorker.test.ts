...
import { useUltimateOscWorker } from './useUltimateOscWorker';

describe('useUltimateOscWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => useUltimateOscWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => useUltimateOscWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => useUltimateOscWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => useUltimateOscWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => useUltimateOscWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => useUltimateOscWorker(mockKlines, enabled), {
  ...
  it('should accept custom periods', () => {
    expect(() => renderHook(() => useUltimateOscWorker(mockKlines, true, 5, 10, 20))).not.toThrow();
  });
});
