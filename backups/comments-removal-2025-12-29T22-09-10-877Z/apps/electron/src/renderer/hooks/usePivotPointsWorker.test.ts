...
import { usePivotPointsWorker } from './usePivotPointsWorker';

describe('usePivotPointsWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => usePivotPointsWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => usePivotPointsWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => usePivotPointsWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => usePivotPointsWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => usePivotPointsWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => usePivotPointsWorker(mockKlines, enabled), {
  ...
  it('should accept custom lookback', () => {
    expect(() => renderHook(() => usePivotPointsWorker(mockKlines, true, 10))).not.toThrow();
  });
});
