...
import { useTSIWorker } from './useTSIWorker';

describe('useTSIWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => useTSIWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => useTSIWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => useTSIWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => useTSIWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => useTSIWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => useTSIWorker(mockKlines, enabled), {
  ...
  it('should accept custom periods', () => {
    expect(() => renderHook(() => useTSIWorker(mockKlines, true, 30, 15, 10))).not.toThrow();
  });
});
