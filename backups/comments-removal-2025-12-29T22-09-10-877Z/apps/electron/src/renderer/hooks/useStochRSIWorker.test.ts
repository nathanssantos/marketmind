...
import { useStochRSIWorker } from './useStochRSIWorker';

describe('useStochRSIWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => useStochRSIWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => useStochRSIWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => useStochRSIWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => useStochRSIWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => useStochRSIWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => useStochRSIWorker(mockKlines, enabled), {
  ...
  it('should accept custom periods', () => {
    expect(() => renderHook(() => useStochRSIWorker(mockKlines, true, 14, 14, 3, 3))).not.toThrow();
  });
});
