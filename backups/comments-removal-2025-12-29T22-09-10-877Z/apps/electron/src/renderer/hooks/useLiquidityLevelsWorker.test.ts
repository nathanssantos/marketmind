...
import { useLiquidityLevelsWorker } from './useLiquidityLevelsWorker';

describe('useLiquidityLevelsWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => useLiquidityLevelsWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => useLiquidityLevelsWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => useLiquidityLevelsWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => useLiquidityLevelsWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => useLiquidityLevelsWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => useLiquidityLevelsWorker(mockKlines, enabled), {
  ...
  it('should accept custom lookback and threshold', () => {
    expect(() => renderHook(() => useLiquidityLevelsWorker(mockKlines, true, 100, 5))).not.toThrow();
  });
});
