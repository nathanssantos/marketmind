...
export class Setup94Detector extends BaseSetupDetector {
  ...
  private calculateVolumeConfirmation(
    ...
    const volumeConfirmation = current.volume >= avgVolume * this.setup94Config.volumeMultiplier;
  ...
  private createLongSetup(
    ...
    const failureLow = previous.low * STOP_BUFFER_LONG;
  ...
  private createShortSetup(
    ...
    const failureHigh = previous.high * STOP_BUFFER_SHORT;
  ...
  detect(candles: Candle[], currentIndex: number): SetupDetectorResult {
    ...
    const ema9 = calculateEMA(candles, this.setup94Config.emaPeriod);
    const atr = calculateATR(candles, this.setup94Config.atrPeriod);
    const current = candles[currentIndex];
    const previous = candles[currentIndex - 1];
    const twoPrev = candles[currentIndex - LOOKBACK_TWO_PREV];
    ...
    if (wasUptrend && turnedDown && resumedUp && lowNotLost) {
      return this.createLongSetup(candles, currentIndex, current, previous, ema9Current, atrCurrent, avgVolume, volumeConfirmation);
    }
    ...
    if (wasDowntrend && turnedUp && resumedDown && highNotLost) {
      return this.createShortSetup(candles, currentIndex, current, previous, ema9Current, atrCurrent, avgVolume, volumeConfirmation);
    }
  ...
  private calculateConfidence(
    ...
    const resumptionStrength = isLong
      ? (current.high - previous.high) / previous.high
      : (previous.low - current.low) / previous.low;
    
    if (resumptionStrength > CANDLE_STRONG_THRESHOLD) boost += CONFIDENCE_BOOST_MEDIUM;
    else if (resumptionStrength > CANDLE_MODERATE_THRESHOLD) boost += CONFIDENCE_BOOST_SMALL;
