...
| `PanelHeader` | **Dashboard-style panel header (with `borderBottom`)** — title + optional description + optional action slot. Use in AnalyticsDialog panels and any data-display panel where the title needs visual separation from the body. Same title typography as `FormSection` (sm/semibold). |
