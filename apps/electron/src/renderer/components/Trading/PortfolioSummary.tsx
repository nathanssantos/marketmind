...
const PortfolioSummaryComponent = ({
  ...
  // Each metrics row sits on its own Flex separated by a 1px divider
  // matching the existing stopProtected/tpProjected separator style.
  // The card surface (bg=bg.surface, p=3, borderRadius) was dropped per
  // user request — the row separators carry the visual grouping.
  const divider = <Box h="1px" w="100%" bg="fg.muted" opacity={0.2} />;

  return (
    <Stack gap={1} fontSize="xs" flex={1} minH={0}>
