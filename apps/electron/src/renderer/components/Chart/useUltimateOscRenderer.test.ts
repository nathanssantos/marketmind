...
describe('useUltimateOscRenderer', () => {
  ...
  const mockColors = {
    ...
    wick: '#838383',
    ultimateOsc: {
      line: 'rgba(255, 193, 7, 0.8)',
    },
  };

  const mockUltimateOscData = {
    values: [55, 60, 65, 70, 45],
  };
  ...
  describe('render', () => {
    it('should not render when manager is null', () => {
      const { result } = renderHook(() =>
        useUltimateOscRenderer({
          manager: null,
          ultimateOscData: mockUltimateOscData,
          colors: mockColors,
          enabled: true,
        })
      );
    ...
    it('should not render when ultimateOscData is null', () => {
      const { result } = renderHook(() =>
        useUltimateOscRenderer({
          manager: mockManager,
          ultimateOscData: null,
          colors: mockColors,
          enabled: true,
        })
      );
    ...
    it('should render Ultimate Oscillator when enabled with valid data', () => {
      const { result } = renderHook(() =>
        useUltimateOscRenderer({
          manager: mockManager,
          ultimateOscData: mockUltimateOscData,
