...
describe('useSupertrendRenderer', () => {
  ...
  const mockColors = {
    ...
    text: '#D9D9D9',
    axisLabel: '#848E9C',
    supertrend: { up: '#26a69a', down: '#ef5350' },
  };

  const mockSupertrendData = {
    value: [98, 97, 96, 104, 105],
  ...
  describe('render', () => {
    it('should not render when manager is null', () => {
      const { result } = renderHook(() => useSupertrendRenderer({ manager: null, supertrendData: mockSupertrendData, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should not render when disabled', () => {
      const { result } = renderHook(() => useSupertrendRenderer({ manager: mockManager, supertrendData: mockSupertrendData, colors: mockColors, enabled: false }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should not render when supertrendData is null', () => {
      const { result } = renderHook(() => useSupertrendRenderer({ manager: mockManager, supertrendData: null, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.stroke).not.toHaveBeenCalled();
    });

    it('should render Supertrend when enabled with valid data', () => {
      const { result } = renderHook(() => useSupertrendRenderer({ manager: mockManager, supertrendData: mockSupertrendData, colors: mockColors, enabled: true }));
      result.current.render();
  ...
  describe('default values', () => {
    it('should use default enabled of true when not specified', () => {
      const { result } = renderHook(() => useSupertrendRenderer({ manager: mockManager, supertrendData: mockSupertrendData, colors: mockColors }));
      result.current.render();
      expect(mockCtx.save).toHaveBeenCalled();
    });
  });
