...
describe('useAORenderer', () => {
  ...
  const mockColors = {
    ...
    text: '#D9D9D9',
    axisLabel: '#848E9C',
    ao: { bullish: 'rgba(38, 166, 154, 0.8)', bearish: 'rgba(239, 83, 80, 0.8)' },
  };

  const mockAoData = { values: [0.5, -0.3, 0.8, -0.2, 0.6] };
  ...
  describe('render', () => {
    it('should not render when manager is null', () => {
      const { result } = renderHook(() => useAORenderer({ manager: null, aoData: mockAoData, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.fillRect).not.toHaveBeenCalled();
    });

    it('should not render when disabled', () => {
      const { result } = renderHook(() => useAORenderer({ manager: mockManager, aoData: mockAoData, colors: mockColors, enabled: false }));
      result.current.render();
      expect(mockCtx.fillRect).not.toHaveBeenCalled();
    });

    it('should not render when aoData is null', () => {
      const { result } = renderHook(() => useAORenderer({ manager: mockManager, aoData: null, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.fillRect).not.toHaveBeenCalled();
    });

    it('should render AO when enabled with valid data', () => {
      const { result } = renderHook(() => useAORenderer({ manager: mockManager, aoData: mockAoData, colors: mockColors, enabled: true }));
      result.current.render();
  ...
  describe('default values', () => {
    it('should use default enabled of true when not specified', () => {
      const { result } = renderHook(() => useAORenderer({ manager: mockManager, aoData: mockAoData, colors: mockColors }));
      result.current.render();
      expect(mockCtx.save).toHaveBeenCalled();
    });
  });
