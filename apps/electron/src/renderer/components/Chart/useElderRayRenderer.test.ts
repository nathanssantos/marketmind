...
describe('useElderRayRenderer', () => {
  ...
  const mockColors = {
    ...
    text: '#D9D9D9',
    axisLabel: '#848E9C',
    elderRay: { bullPower: '#26a69a', bearPower: '#ef5350' },
  };

  const mockElderRayData = { bullPower: [5, 8, 3, -2, 6], bearPower: [-3, -5, -2, -8, -4] };
  ...
  describe('render', () => {
    it('should not render when manager is null', () => {
      const { result } = renderHook(() => useElderRayRenderer({ manager: null, elderRayData: mockElderRayData, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.fillRect).not.toHaveBeenCalled();
    });

    it('should not render when disabled', () => {
      const { result } = renderHook(() => useElderRayRenderer({ manager: mockManager, elderRayData: mockElderRayData, colors: mockColors, enabled: false }));
      result.current.render();
      expect(mockCtx.fillRect).not.toHaveBeenCalled();
    });

    it('should not render when elderRayData is null', () => {
      const { result } = renderHook(() => useElderRayRenderer({ manager: mockManager, elderRayData: null, colors: mockColors, enabled: true }));
      result.current.render();
      expect(mockCtx.fillRect).not.toHaveBeenCalled();
    });

    it('should render Elder Ray when enabled with valid data', () => {
      const { result } = renderHook(() => useElderRayRenderer({ manager: mockManager, elderRayData: mockElderRayData, colors: mockColors, enabled: true }));
      result.current.render();
  ...
  describe('default values', () => {
    it('should use default enabled of true when not specified', () => {
      const { result } = renderHook(() => useElderRayRenderer({ manager: mockManager, elderRayData: mockElderRayData, colors: mockColors }));
      result.current.render();
      expect(mockCtx.save).toHaveBeenCalled();
    });
  });
