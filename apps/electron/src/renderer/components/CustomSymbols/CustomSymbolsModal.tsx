...
export const CustomSymbolsModal = memo(() => {
  const { t } = useTranslation();
  const { isCustomSymbolsOpen, setCustomSymbolsOpen } = useUIStore(
    useShallow((s) => ({
      isCustomSymbolsOpen: s.isCustomSymbolsOpen,
      setCustomSymbolsOpen: s.setCustomSymbolsOpen,
    }))
  );
  ...
  const handleClose = useCallback(() => setCustomSymbolsOpen(false), [setCustomSymbolsOpen]);
  ...
  const updateComponent = (index: number, field: keyof ComponentInput, value: string) => {
    setFormComponents(prev => prev.map((c, i) => i === index ? { ...c, [field]: field === 'symbol' ? value.toUpperCase() : value } : c));
  };
  ...
  return (
    <DialogRoot open={isCustomSymbolsOpen} onOpenChange={handleOpenChange} size="xl">
      <DialogBackdrop />
      <DialogPositioner>
        <DialogContent maxH="90vh" maxW="900px" w="90vw">
          <DialogHeader px={4} pt={4} pb={3}>
            <Flex justify="space-between" align="center" w="100%">
              <DialogTitle fontSize="md">{t('customSymbols.title')}</DialogTitle>
              <IconButton
                size="2xs"
                variant="ghost"
...
CustomSymbolsModal.displayName = 'CustomSymbolsModal';
