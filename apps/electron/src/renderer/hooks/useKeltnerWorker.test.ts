...
import { useKeltnerWorker } from './useKeltnerWorker';

describe('useKeltnerWorker', () => {
  const mockKlines = [
  ...
  it('should return null when disabled', () => {
    const { result } = renderHook(() => useKeltnerWorker(mockKlines, false));
    expect(result.current).toBeNull();
  });

  it('should return null when klines are empty', () => {
    const { result } = renderHook(() => useKeltnerWorker([], true));
    expect(result.current).toBeNull();
  });

  it('should initialize hook without throwing', () => {
    expect(() => renderHook(() => useKeltnerWorker(mockKlines, true))).not.toThrow();
  });

  it('should cleanup on unmount', () => {
    const { unmount } = renderHook(() => useKeltnerWorker(mockKlines, true));
    expect(() => unmount()).not.toThrow();
  });

  it('should handle re-render with new klines', () => {
    const { rerender } = renderHook(({ klines }) => useKeltnerWorker(klines, true), {
  ...
  it('should handle toggle from disabled to enabled', () => {
    const { rerender } = renderHook(({ enabled }) => useKeltnerWorker(mockKlines, enabled), {
  ...
  it('should accept custom periods', () => {
    expect(() => renderHook(() => useKeltnerWorker(mockKlines, true, 20, 10, 2))).not.toThrow();
  });
});
