...
export const computeSingle = async (
  type: SingleIndicatorType,
  klines: Kline[],
  ...
  if (!scriptFn) throw new Error(`Unsupported indicator: ${type}`);
  const source = `//@version=5\nindicator("ind")\nplot(${scriptFn(params)}, "r")`;
  const plots = await runPine(klines, source);
  return extractPlotValues(plots, 'r');
};

export const computeMulti = async (
  type: MultiIndicatorType,
  klines: Kline[],
  params: Record<string, number> = {},
  ...
  for (const def of plotDefs) {
    ...
    result[name!] = extractPlotValues(plots, name!);
  }
  return result;
};

export const computeOBV = async (
  klines: Kline[],
  smaPeriod?: number,
): Promise<{ values: (number | null)[]; sma: (number | null)[] }> => {
  if (klines.length === 0) return { values: [], sma: [] };
  const smaLine = smaPeriod ? `\nplot(ta.sma(_obv, ${smaPeriod}), "sma")` : '';
  const source = `//@version=5\nindicator("ind")\n_obv = ta.obv()\nplot(_obv, "obv")${smaLine}`;
  const plots = await runPine(klines, source);
  return {
    values: extractPlotValues(plots, 'obv'),
    sma: smaPeriod ? extractPlotValues(plots, 'sma') : [],
  };
};

export const computeTSI = async (
  klines: Kline[],
  longPeriod = 25,
  shortPeriod = 13,
  signalPeriod = 13,
): Promise<{ tsi: (number | null)[]; signal: (number | null)[] }> => {
  if (klines.length === 0) return { tsi: [], signal: [] };
  const source = `//@version=5\nindicator("ind")\n_tsi = ta.tsi(close, ${shortPeriod}, ${longPeriod})\nplot(_tsi, "tsi")\nplot(ta.ema(_tsi, ${signalPeriod}), "signal")`;
  const plots = await runPine(klines, source);
  return {
    tsi: extractPlotValues(plots, 'tsi'),
    signal: extractPlotValues(plots, 'signal'),
  };
};

export const computeSupertrend = async (
  klines: Kline[],
  period: number,
  multiplier: number,
): Promise<{ value: (number | null)[]; trend: ('up' | 'down' | null)[] }> => {
  if (klines.length === 0) return { value: [], trend: [] };
  const raw = await computeMulti('supertrend', klines, { period, multiplier });
  const directionArr = raw['direction'] ?? [];
  return {
    value: raw['value'] ?? [],
    trend: directionArr.map((d) => d === null ? null : d < 0 ? 'up' : 'down'),
  };
};
