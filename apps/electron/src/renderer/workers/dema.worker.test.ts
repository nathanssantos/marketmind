...
vi.mock('../lib/indicators', () => ({
  calculateDEMA: vi.fn(() => ({ values: [100, 101, 102] })),
}));

describe('dema.worker', () => {
  ...
  it('should calculate DEMA with default period and post result', async () => {
    await import('./dema.worker');
    ...
    expect(calculateDEMA).toHaveBeenCalledWith(mockKlines, 21);
  ...
  it('should use custom period when provided', async () => {
    ...
    expect(calculateDEMA).toHaveBeenCalledWith(mockKlines, 14);
  });
});
