...
describe('cmf.worker', () => {
  ...
  it('should calculate CMF with default period and post result', async () => {
    await import('./cmf.worker');
    ...
    expect(calculateCMF).toHaveBeenCalledWith(mockKlines, 20);
  ...
  it('should use custom period when provided', async () => {
    ...
    expect(calculateCMF).toHaveBeenCalledWith(mockKlines, 14);
  });
});
