...
describe('tema.worker', () => {
  ...
  it('should calculate TEMA with default period', async () => {
    await import('./tema.worker');
    ...
    expect(calculateTEMA).toHaveBeenCalledWith(mockKlines, 21);
  });
  ...
  it('should use custom period', async () => {
    ...
    expect(calculateTEMA).toHaveBeenCalledWith(mockKlines, 14);
  });
});
