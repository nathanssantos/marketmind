...
export const optimizeKlines = (klines: Kline[], detailedCount: number = DETAILED_KLINES_COUNT): OptimizedKlineData => {
  if (klines.length === 0) {
    return {
      detailed: [],
      simplified: [],
  ...
  const remainingKlines = klines.slice(0, -DETAILED_KLINES_COUNT);
...
export const formatKlinesForPrompt = (optimized: OptimizedKlineData): string => {
  const parts: string[] = [];
  
  if (optimized.detailed.length > 0) {
    parts.push('📊 Recent Klines (MOST CURRENT - Use these timestamps for current price levels):');
    optimized.detailed.forEach((kline, index) => {
      ...
      parts.push(
        `${index + 1}. [${date}] O:$${getKlineOpen(kline).toFixed(2)} H:$${getKlineHigh(kline).toFixed(2)} ` +
        `L:$${getKlineLow(kline).toFixed(2)} C:$${getKlineClose(kline).toFixed(2)} V:${getKlineVolume(kline).toLocaleString()}${marker}`
      );
    });
  }
  
  if (optimized.simplified.length > 0) {
    parts.push(`\n📈 Historical Data (${optimized.simplified.length} klines, simplified - OLDER data):`);
    ...
    if (firstSimplified && lastSimplified) {
      parts.push(
        `Range: ${new Date(firstSimplified.openTime).toISOString()} to ` +
        `${new Date(lastSimplified.openTime).toISOString()}`
      );
    }
    
    const sample = optimized.simplified.filter((_, i) => i % Math.ceil(optimized.simplified.length / 5) === 0);
    sample.forEach((kline) => {
      const date = new Date(kline.openTime).toISOString().split('T')[0];
      const closePrice = typeof kline.close === 'string' ? parseFloat(kline.close) : kline.close;
      parts.push(`  ${date}: $${closePrice.toFixed(2)}`);
    });
  }
