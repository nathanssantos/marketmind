...
export class OrderBlockFVGDetector extends BaseSetupDetector {
  ...
  detect(klines: Kline[], currentIndex: number): SetupDetectorResult {
    const minIndex =
      this.orderBlockFVGConfig.lookbackPeriod + VOLUME_LOOKBACK;
  ...
  private findBullishOrderBlock(
    ...
    for (let i = currentIndex - 1; i >= lookbackStart; i -= 1) {
      ...
      if (isBullish && hasHighVolume) {
        ...
        if (
          currentPrice >= getKlineLow(kline) &&
          currentPrice <= getKlineHigh(kline)
        ) {
          return {
            type: 'bullish',
...
export const createDefaultOrderBlockFVGConfig = (): OrderBlockFVGConfig => ({
  enabled: false,
  minConfidence: 75,
  minRiskReward: 2.0,
  fvgMinSize: 0.1,
