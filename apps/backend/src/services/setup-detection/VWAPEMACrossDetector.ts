...
export class VWAPEMACrossDetector extends BaseSetupDetector {
  ...
  detect(
    ...
    const vwapValues = calculateVWAP(lookbackKlines);
    ...
    const bullishCross = this.detectBullishCross(
      klines,
      currentIndex,
      vwap,
      currentEMA,
    ...
    if (bullishCross) {
      const setup = this.createBullishSetup(
        klines,
        currentIndex,
        vwap,
        currentEMA,
      ...
      if (setup) {
        ...
        if (rr >= this.config.minRiskReward) {
          const confidence = this.calculateConfidence(
            klines,
            currentIndex,
            bullishCross.hasVolumeConfirmation,
          );
    ...
    const bearishCross = this.detectBearishCross(
      klines,
      currentIndex,
      vwap,
      currentEMA,
    ...
    if (bearishCross) {
      const setup = this.createBearishSetup(
        klines,
        currentIndex,
        vwap,
        currentEMA,
      ...
      if (setup) {
        ...
        if (rr >= this.config.minRiskReward) {
          const confidence = this.calculateConfidence(
            klines,
            currentIndex,
            bearishCross.hasVolumeConfirmation,
          );
          return { setup, confidence };
        }
      }
  ...
  private detectBullishCross(
    ...
    for (
      ...
      i >= Math.max(0, currentIndex - CROSS_LOOKBACK);
      i -= 1
    ) {
      ...
      const emaValues = calculateEMA(
        klines.slice(0, i + 1),
        vwapConfig.emaPeriod,
      );
      const ema = emaValues[emaValues.length - 1];
      const prevEmaValues = calculateEMA(
        klines.slice(0, i),
        vwapConfig.emaPeriod,
      );
      ...
      const pullbackKline = this.findBullishPullback(
        klines,
        i,
        currentIndex,
        vwap,
      ...
      const hasVolumeConfirmation = this.hasVolumeConfirmation(
        klines,
        currentIndex,
      );
  ...
  private detectBearishCross(
    ...
    for (
      ...
      const pullbackKline = this.findBearishPullback(
        klines,
        i,
        currentIndex,
        vwap,
  ...
  private findBullishPullback(
    ...
    const toleranceAmount =
      vwap * (vwapConfig.pullbackTolerance / PERCENT_DIVISOR);
    ...
    for (let i = crossIndex + 1; i <= currentIndex; i += 1) {
      ...
      const touchedVWAP =
        getKlineLow(kline) <= vwap + toleranceAmount &&
        getKlineLow(kline) >= pullbackTarget;
  ...
  private findBearishPullback(
    ...
    for (let i = crossIndex + 1; i <= currentIndex; i += 1) {
      ...
      const touchedVWAP =
        getKlineHigh(kline) >= vwap - toleranceAmount &&
        getKlineHigh(kline) <= pullbackTarget;
  ...
  private createBullishSetup(
    ...
    const rrTarget =
      entry + risk * (this.config as VWAPEMACrossConfig).targetMultiplier;
    const target =
  ...
  private calculateConfidence(
    ...
    const klineSize = Math.abs(
      getKlineClose(currentKline) - getKlineOpen(currentKline),
    );
  ...
  private calculateATR(
    ...
    for (
      let i = Math.max(1, currentIndex - period + 1);
      i <= currentIndex;
      i += 1
    ) {
