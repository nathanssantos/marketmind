...
export class BreakoutRetestDetector extends BaseSetupDetector {
  ...
  detect(klines: Kline[], currentIndex: number): SetupDetectorResult {
    const minIndex = Math.max(
      this.breakoutRetestConfig.lookbackPeriod +
        this.breakoutRetestConfig.emaPeriod,
    ...
    const bearishSetup = this.detectBearishBreakoutRetest(
      klines,
      currentIndex,
    );
  ...
  private detectBullishBreakoutRetest(
    ...
    for (const resistance of resistanceLevels) {
      const breakoutKline = this.findBreakoutKline(
        klines,
        resistance,
        currentIndex,
        'bullish',
      ...
      const continuationConfirmed =
        getKlineClose(current) >
        resistance *
          (1 + MIN_CONTINUATION_STRENGTH * CONTINUATION_PERCENT_MULTIPLIER);
  ...
  private findResistanceLevels(
    ...
    const lookbackStart = Math.max(
      0,
      currentIndex - this.breakoutRetestConfig.lookbackPeriod,
    );
    const highPivots = pivots
      .filter((p) => p.type === 'high')
      .filter(
        (p) => p.index >= lookbackStart && p.index < currentIndex - PIVOT_LOOKBACK,
      );
  ...
  private findBreakoutKline(
    ...
    for (let i = lookbackStart; i < currentIndex; i++) {
      ...
      const isBullishBreakout =
        direction === 'bullish' && getKlineClose(kline) > level;
      const isBearishBreakout =
        direction === 'bearish' && getKlineClose(kline) < level;
  ...
  private validateRetest(
    ...
    for (let i = breakoutIndex + 1; i <= currentIndex; i++) {
      ...
      const isRetestTouch =
        direction === 'bullish'
          ? getKlineLow(kline) <= level + tolerance &&
            getKlineLow(kline) >= level - tolerance
          : getKlineHigh(kline) >= level - tolerance &&
            getKlineHigh(kline) <= level + tolerance;
      ...
      if (isRetestTouch) {
        ...
        const holdQuality =
          direction === 'bullish'
            ? (getKlineClose(kline) - getKlineLow(kline)) /
              (getKlineHigh(kline) - getKlineLow(kline))
            : (getKlineHigh(kline) - getKlineClose(kline)) /
              (getKlineHigh(kline) - getKlineLow(kline));
  ...
  private createBullishSetup(
    ...
    const volumeConfirmation =
      getKlineVolume(current) >
      avgVolume * this.breakoutRetestConfig.volumeMultiplier;
    ...
    const swingLow = findLowestSwingLow(
      klines,
      currentIndex,
      this.breakoutRetestConfig.lookbackPeriod,
      3,
    ...
    if (!this.meetsMinimumRequirements(confidence, rr)) return null;
  ...
  private createBearishSetup(
    ...
    const swingHigh = findHighestSwingHigh(
      klines,
      currentIndex,
      this.breakoutRetestConfig.lookbackPeriod,
      3,
  ...
  private calculateConfidence(
    ...
    const breakoutBonus =
      Math.min(breakoutStrength * BREAKOUT_STRENGTH_SCALE, 1) *
    ...
    const totalConfidence =
      baseConfidence + breakoutBonus + retestBonus + volumeBonus + emaBonus;
