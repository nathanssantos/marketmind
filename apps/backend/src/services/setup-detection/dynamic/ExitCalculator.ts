...
import { calculateSwingHighLowStop } from './exit-swing-helpers';
import { calculatePivotBasedStop, calculatePivotBasedTarget } from './exit-pivot-helpers';
...
export class ExitCalculator {
  ...
  calculateStopLoss(exit: ExitLevel, context: ExitContext): number {
    ...
    if (exit.type === 'swingHighLow') {
      stopLoss = calculateSwingHighLowStop(
        exit, context, this.indicatorEngine,
        (operand, ctx) => this.resolveOperand(operand, ctx)
      );
    } else if (exit.type === 'pivotBased') {
      stopLoss = calculatePivotBasedStop(
        exit, context, this.indicatorEngine,
        (operand, ctx) => this.resolveOperand(operand, ctx),
        (e, ctx) => calculateSwingHighLowStop(
          e, ctx, this.indicatorEngine,
          (operand, c) => this.resolveOperand(operand, c)
        )
      );
    } else {
      ...
      if (distance < 0) {
        logger.warn({
          direction, entryPrice, distance, exitType: exit.type,
  ...
  calculateTakeProfit(
    ...
    if (exit.type === 'pivotBased') {
      return calculatePivotBasedTarget(
        exit, context, this.indicatorEngine, stopLossPrice,
        (e, ctx, slp) => this.calculateTakeProfit(e, ctx, slp)
      );
    }
  ...
  private getATRValue(exit: ExitLevel, context: ExitContext): number {
    ...
    const value = this.indicatorEngine.resolveIndicatorValue(
      indicators, indicatorRef, currentIndex
    );
  ...
  private resolveIndicatorTarget(
    ...
    if (indicatorRef === undefined) return null;

    if (typeof indicatorRef === 'number') return indicatorRef;
  ...
  resolveOperand(
    ...
    if (typeof operand === 'string') {
      const value = this.indicatorEngine.resolveIndicatorValue(
        context.indicators, operand, context.currentIndex
      );
      return value ?? 0;
    }
  ...
  calculateConfidence(
    ...
    if (!config) return this.calculateDefaultConfidence(context);
