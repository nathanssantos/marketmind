...
export class DivergenceDetector extends BaseSetupDetector {
  ...
  private tryDetectRSIDivergence(
    ...
    const bullishRSIDivergence = this.detectBullishRSIDivergence(
      klines,
      currentIndex,
    );
    ...
    const bearishRSIDivergence = this.detectBearishRSIDivergence(
      klines,
      currentIndex,
    );
  ...
  private tryDetectMACDDivergence(
    ...
    const bullishMACDDivergence = this.detectBullishMACDDivergence(
      klines,
      currentIndex,
    );
    ...
    const bearishMACDDivergence = this.detectBearishMACDDivergence(
      klines,
      currentIndex,
    );
  ...
  private detectBullishRSIDivergence(
    ...
    const startIndex = Math.max(
      0,
      currentIndex - divConfig.divergenceLookback,
    );
  ...
  private createBearishSetup(
    ...
    const stop = Math.max(
      firstHighKline
        ? getKlineHigh(firstHighKline)
        : getKlineHigh(currentKline),
      secondHighKline
        ? getKlineHigh(secondHighKline)
        : getKlineHigh(currentKline),
    );
