...
export class PinInsideDetector extends BaseSetupDetector {
  ...
  detect(klines: Kline[], currentIndex: number): SetupDetectorResult {
    const minIndex =
      this.pinInsideConfig.lookbackPeriod + VOLUME_LOOKBACK;

    if (
      !this.config.enabled ||
      currentIndex < minIndex ||
      currentIndex < MIN_INDEX_OFFSET
    ) {
  ...
  private isPinBar(kline: Kline): {
    isPinBar: boolean;
    type: 'bullish' | 'bearish' | null;
  } {
    const body = Math.abs(getKlineClose(kline) - getKlineOpen(kline));
    const wickLow = Math.abs(
      getKlineLow(kline) - Math.min(getKlineOpen(kline), getKlineClose(kline)),
    );
    const wickHigh = Math.abs(
      getKlineHigh(kline) -
        Math.max(getKlineOpen(kline), getKlineClose(kline)),
    );
  ...
  private isInsideBar(motherKline: Kline, insideKline: Kline): boolean {
    return (
      parseFloat(insideKline.high) <= parseFloat(motherKline.high) &&
      parseFloat(insideKline.low) >= parseFloat(motherKline.low)
    );
  }
  ...
  private findNearbySupportResistance(
    ...
    const lookbackStart = Math.max(
      0,
      pinBarIndex - this.pinInsideConfig.lookbackPeriod,
    );
    ...
    const tolerance =
      getKlineClose(pinBarKline) *
      (this.pinInsideConfig.srTolerance / PERCENT_DIVISOR);

    for (const pivot of pivots) {
      const isNearLow =
        Math.abs(getKlineLow(pinBarKline) - pivot.price) < tolerance;
      const isNearHigh =
        Math.abs(getKlineHigh(pinBarKline) - pivot.price) < tolerance;
  ...
  private checkVolumeConfirmation(
    ...
    const avgVolume =
      recentKlines.reduce((sum, c) => sum + getKlineVolume(c), 0) /
      recentKlines.length;
  ...
  private createBullishSetup(
    ...
    const structuralTarget =
      resistance && resistance < rrTarget
        ? resistance * TARGET_BUFFER_LONG
        : rrTarget;
    const takeProfit = Math.max(
      structuralTarget,
      entry + risk * this.pinInsideConfig.targetMultiplier,
    );

    const rr = this.calculateRR(entry, stopLoss, takeProfit);

    if (!this.meetsMinimumRequirements(BASE_CONFIDENCE, rr)) {
      return null;
    }
  ...
  private createBearishSetup(
    ...
    const rrTarget = entry - risk * this.pinInsideConfig.targetMultiplier;
    const structuralTarget =
      support && support > rrTarget ? support * TARGET_BUFFER_SHORT : rrTarget;
  ...
  private findNearestResistance(
    ...
    const lookback = Math.min(
      this.pinInsideConfig.lookbackPeriod,
      currentIndex,
    );
