...
program
  ...
  .description('Optimize full trading system (pyramiding + trailing stop)')
  .requiredOption('-s, --strategy <type>', 'Strategy to optimize (e.g., setup91, setup92)')
  .requiredOption('--symbol <symbol>', 'Trading symbol (e.g., BTCUSDT, ETHUSDT, SOLUSDT)')
  .requiredOption('-i, --interval <interval>', 'Timeframe (e.g., 1h, 4h, 1d)')
  .requiredOption('--start <date>', 'Start date (YYYY-MM-DD)')
...
program
  .command('batch')
  .description('Run batch backtests across all strategies, symbols, and timeframes')
  .requiredOption('--start <date>', 'Start date (YYYY-MM-DD)')
  .requiredOption('--end <date>', 'End date (YYYY-MM-DD)')
  .option('-c, --capital <amount>', 'Initial capital in USD', '10000')
  .option('--parallel <n>', 'Number of parallel workers', '4')
  .option('--strategies <list>', 'Comma-separated strategy IDs (default: all strategies)')
  ...
  .option('--no-use-btc-correlation-filter', 'Disable BTC correlation filter')
  .option('--use-market-regime-filter', 'Enable market regime filter', true)
  .option('--no-use-market-regime-filter', 'Disable market regime filter')
  .option('--use-funding-filter', 'Enable funding rate filter (FUTURES only)', true)
  .option('--no-use-funding-filter', 'Disable funding rate filter')
  .option('--use-confluence-scoring', 'Enable confluence scoring', true)
  .option('--no-use-confluence-scoring', 'Disable confluence scoring')
  .option('--use-momentum-timing-filter', 'Enable momentum timing filter', true)
  .option('--no-use-momentum-timing-filter', 'Disable momentum timing filter')
  .option('--include-disabled', 'Include disabled strategies', false)
  .option('-v, --verbose', 'Show detailed logs', false)
