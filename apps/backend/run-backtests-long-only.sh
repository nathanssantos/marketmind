...
echo "=== MarketMind Backtests (LONG ONLY) ==="
echo "Date: $(date)"
echo "Strategies: 12"
echo "Symbols: 4"
echo "Intervals: 6"
...
RESULTS_FILE="backtest-results-long-only-$(date +%Y%m%d-%H%M%S).csv"
...
for symbol in $SYMBOLS; do
  for interval in $INTERVALS; do
    ...
    for strategy in $STRATEGIES; do
      ...
      result=$(timeout 120 pnpm exec tsx src/cli/backtest-runner.ts validate -s "$strategy" --symbol "$symbol" -i "$interval" --start 2024-01-01 --end 2024-12-01 --optimized --only-long 2>&1)
