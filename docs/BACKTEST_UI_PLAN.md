...
## Reuse inventory — do NOT recreate these

The audit before writing this plan confirmed every item below already exists. Wire to it; do not fork or reimplement.

**Backend / engine / tRPC**
- `setupDetection.listStrategies` (`apps/backend/src/routers/setup-detection.ts:47-72`) — returns Pine metadata `{ id, name, version, description, author, tags, status, enabled, recommendedTimeframes }[]`, supports `includeStatuses`/`excludeStatuses` filter. The Strategies tab consumes this.
- `setupDetection.getStrategyDetails` (`:74-85`) — full metadata for one strategy. Use in the optional details drawer.
- `setupDetection.getStrategyEducation` (`:235-249`) — `StrategyEducation` payload (overview, signals, risk notes). Same drawer.
- `backtest.simple.run` / `backtest.simple.getResult` / `backtest.simple.list` (`apps/backend/src/routers/backtest/simple.ts`) — already wired. Wave 1 reshapes `run` to return `{ backtestId }` (fire-and-forget); `getResult` already exists for fetching cached output by id.
- `backtest.multiWatcher` (`apps/backend/src/routers/backtest/multi.ts`) — for the multi-symbol path (later wave or out of scope; surface as a separate modal if it ever ships).
- `BacktestOptimizer.onProgress(current, total)` and `FullSystemOptimizer.onProgress(current, total, currentBest?)` — **the canonical progress contract already exists** in the codebase. Wave 1's `BacktestProgressReporter` should be the same shape (`onProgress(current, total)`) so the optimizers can adopt it later for free; the reporter then forwards to socketBus.
- All 105 strategies are Pine Script (`apps/backend/strategies/builtin/*.pine`); `PineStrategyLoader` (`apps/backend/src/services/pine/PineStrategyLoader.ts`) parses metadata. No JSON strategies.
- 19 filter toggles + sub-params live in `apps/backend/src/utils/filters/filter-registry.ts:42-243` — Wave 0 reflects that registry into a typed `FILTER_DEFINITIONS` const.

**Frontend components (`apps/electron/src/renderer/components/`)**
- `SymbolSelector.tsx` — symbol typeahead. Use in BasicTab.
- `ui/MetricCard.tsx` — labelled metric card. Use throughout the results panel.
- `ui/FormDialog.tsx` — modal shell with cancel/submit. Wraps the entire BacktestModal.
- `ui/Tabs`, `ui/CollapsibleSection`, `ui/Field`, `ui/Switch`, `ui/Select` (`usePortal={false}` inside Dialog!), `ui/NumberInput`, `ui/Slider`, `ui/Checkbox`, `ui/RadioGroup`, `ui/Textarea`, `ui/Progress`, `ui/Alert`, `ui/Badge`, `ui/LoadingSpinner` — every primitive needed.
- `Trading/BulkSymbolSelector.tsx` — `string[]` multi-select pattern. Mirror for the strategy checkbox grid.
- `Trading/PerformancePanel.tsx` — wraps `MetricCard` for trading metrics. **Reference layout** for the backtest results summary; do not duplicate.
- `Analytics/EquityCurveChart.tsx` — already renders an equity curve from a tRPC query keyed by walletId. For backtest results, ship a sibling component that takes the data array as a prop instead of querying — the rendering core (lines ~22-165) is reusable.
- `Trading/StrategyInfoPopover.tsx` — `StrategyVisualizationData` renderer with `CandlePatternTable`, `IndicatorValues`, `PerformanceStats` sub-components. Reuse for the optional Wave 6 strategy-details drawer.
- `Settings/SettingsDialog.tsx` + `Trading/CreateWalletDialog.tsx` + `Screener/ScreenerModal.tsx` — modal+tabs reference patterns. Don't reinvent the wrapper.
- `Layout/Toolbar.tsx:233-242` — the screener button block. Clone this exact pattern (TooltipWrapper + ToggleIconButton + zustand store toggle) for the new backtest trigger.

**Stores / hooks**
- `useScreenerStore` — clone shape into `useBacktestModalStore` (the only new store needed).
- `hooks/socket/useSocketEvent.ts` — typed socket subscription with stable handler ref. Use as-is.
- `hooks/useBackendAnalytics.ts` — pattern reference for a hook that orchestrates tRPC + socket events.

**Test infrastructure**
- `e2e/helpers/trpcMock.ts` — extend with `backtest.simple.run` / `backtest.simple.getResult` overrides; pattern from `e2e/specs/wallet-management.spec.ts`.
- `e2e/helpers/socketBridge.ts` `emitSocketEvent(page, event, payload)` — simulates backend events in tests; pattern from `e2e/stream-health.spec.ts:44-56` (closest analog of "open modal → progressive socket updates → assert UI").
- `e2e/helpers/chartTestSetup.ts` `waitForE2EBridge`, `waitForChartReady`, `installConsoleCapture`.

If a future review finds something the next chat reimplemented from this list, that's a process bug — flag and revert.
...
- Strategy taxonomy — **all 105 strategies are Pine Script** (`apps/backend/strategies/builtin/*.pine`); the JSON→Pine migration already happened. **Do NOT create a new tRPC procedure** — `setupDetectionRouter.listStrategies` (`apps/backend/src/routers/setup-detection.ts:47-72`) already returns full Pine metadata extracted by `PineStrategyLoader`: `{ id, name, version, description, author, tags, status, enabled, recommendedTimeframes }[]`. It also accepts `{ includeStatuses?, excludeStatuses? }` filters where status ∈ `'active' | 'experimental' | 'deprecated' | 'unprofitable'`. The modal consumes `trpc.setupDetection.listStrategies.useQuery({ excludeStatuses: ['deprecated', 'unprofitable'] })` and pre-checks the 13 ids in `apps/backend/src/constants/index.ts` `DEFAULT_ENABLED_SETUPS`. `getStrategyDetails` and `getStrategyEducation` are also already there if a "details" drawer ships in a later wave.
...
- Add a thin `BacktestProgressReporter` class in `apps/backend/src/services/backtesting/BacktestProgressReporter.ts`. **The contract mirrors the existing `BacktestOptimizer.onProgress(current, total)` callback** (`apps/backend/src/services/backtesting/BacktestOptimizer.ts:10` — and the same shape in `FullSystemOptimizer.ts:121`) so the reporter can transparently drop in wherever a callback is already passed. Constructor takes `{ backtestId, io, totalSteps }`. Methods: `setPhase(phase, total)`, `tick(processed)` — equivalent to `onProgress(processed, total)`, `complete(resultId)`, `fail(error)`. Internally tracks `startedAt` and computes ETA as `(elapsed / processed) * (total - processed)` — clamped to `null` until at least 5% of steps have run (otherwise the estimate is useless noise). The optimizers can adopt the reporter in a follow-up by passing `(c, t) => reporter.tick(c)` as their `onProgress` — out of scope for this wave but unlocks the same UI for optimizer runs.
...
- Internal layout: `Tabs.Root` (4 tabs) inside the FormDialog body. Tabs:
  1. **Basic** — symbol via the existing `apps/electron/src/renderer/components/SymbolSelector.tsx` (do not roll a new picker), marketType (SPOT/FUTURES Select), interval Select, startDate + endDate (HTML `<input type="date">` wrapped in `Field` — no date picker primitive exists in `ui/`, native input is fine for now), initialCapital NumberInput, leverage NumberInput (only when marketType=FUTURES — conditional render).
  2. **Strategies** — checkbox grid driven by the **existing** `trpc.setupDetection.listStrategies.useQuery({ excludeStatuses: ['deprecated', 'unprofitable'] })` (don't recreate it). Each row shows `name`, `description`, `status` Badge (color: 'active' green / 'experimental' yellow / 'deprecated' grey), `tags`. Pre-select the 13 ids in `apps/backend/src/constants/index.ts` `DEFAULT_ENABLED_SETUPS`. When the user picks an interval that isn't in any selected strategy's `recommendedTimeframes`, surface a soft Alert: "Strategy X recommends 1h/4h; you picked 5m". A "Show experimental" Switch flips the query's `excludeStatuses` to `['deprecated', 'unprofitable']` only. Use the bulk-select pattern from `apps/electron/src/renderer/components/Trading/BulkSymbolSelector.tsx` (Select All / Deselect All buttons + `string[]` state). Selection drives `setupTypes` field of the schema. Optional: clicking a row name opens a strategy-details drawer reusing `apps/electron/src/renderer/components/Trading/StrategyInfoPopover.tsx` + the existing `trpc.setupDetection.getStrategyDetails` / `getStrategyEducation` queries — mark as Wave 6 polish.
...
- Results panel: `apps/electron/src/renderer/components/Backtest/BacktestResults.tsx` composes existing primitives — `MetricCard` from `ui/` for the headline stats (trades count, total PnL %, win rate, max drawdown %, Sharpe, profit factor) laid out like `Trading/PerformancePanel.tsx` (5-card grid). For the equity curve, ship a sibling `BacktestEquityCurveChart.tsx` that takes `equityCurve: { time: number; equity: number }[]` as a prop and reuses the rendering core of `Analytics/EquityCurveChart.tsx` (~lines 22-165, the SVG/canvas drawing logic) — extract the inner render into a shared `EquityCurveSvg` if needed so both wallet-driven and prop-driven variants share code (do this only if the extraction is mechanical; otherwise duplicate the ~50 LOC and add a TODO). Add a "Run another" button that calls `reset()`.
