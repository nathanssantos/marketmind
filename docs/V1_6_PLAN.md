...
| 8 | `ProfileEditorDialog` | ✅ #390 (workflow batch) — Input/Textarea/Select sm→xs (4 instances) for uniform xs density. Already uses `<DialogShell size="xl">` + `<CollapsibleSection>` per group. |
...
| 12 | `ScreenerDialog` | ✅ #390 (workflow batch) — added `description` (was missing). Already used xs density + Spinner panel combo. BetaBadge in title kept. |
| 13 | `AnalyticsDialog` | ✅ #390 (workflow batch) — added `description`. Replaced "no wallet" `<Callout tone="info">` with `<EmptyState>` (bible: empty body uses EmptyState, Callout is for inline messages mid-form). |
| 14 | `BacktestDialog` | ✅ #390 (workflow batch) — added `description`. Multi-step rendering (idle/running/success/failed) preserved as-is — bible's "stepper" rule is for upfront staged forms; here the steps are status-driven. |
| 15 | `SettingsDialog` | ✅ #390 (workflow batch) — added `description`. Vertical tabs with `variant="subtle"` kept (bible's `variant="line"` rule is for horizontal tabs; vertical needs different visual weight). |
